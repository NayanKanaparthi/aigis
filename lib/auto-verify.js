...
const { getUserJurisdictions } = require('./jurisdiction');
...
/**
 * v2.1: Read an area's frontmatter `controls` block (all four frameworks).
 * Returns { owasp, nist, iso42001, eu_ai_act } with arrays (empty if absent).
 * Returns null on missing file or parse failure.
 */
function readAreaControls(areaId) {
  try {
 ...
 */
function autoVerifyArea(areaId, projectDir, opts = {}) {
  // v2.1: opts.jurisdictions = array of jurisdiction codes (e.g. ['eu']).
  // Used to gate EU AI Act control citations in the output. Defaults to none
  // — verify is conservative; eu_ai_act citations only appear when the user
  // explicitly opts in via --jurisdiction eu.
  const userJurisdictions = new Set(opts.jurisdictions || []);
  ...
  // If the area's frontmatter has `jurisdiction: [eu]` and the user is not
  // in EU jurisdiction, skip the entire area citation block (the area
  // shouldn't have been verified at all in that case).
  const allControls = readAreaControls(areaId);
  ...
  // v2.1: assemble the controls citation. EU AI Act controls are gated on
  // the user passing --jurisdiction eu (or equivalent). NIST/OWASP/ISO are
  // always cited if the area declares them.
  const showEu = userJurisdictions.has('eu');
  const cited = allControls ? {
    owasp: allControls.owasp,
    nist: allControls.nist,
    iso42001: allControls.iso42001,
    eu_ai_act: showEu ? allControls.eu_ai_act : [],
  } : null;
...
function formatTextReport(result) {
  ...
  // v2.1: cite framework controls satisfied when ALL auto checks PASS.
  // Citation is gated on user jurisdiction for EU AI Act references.
  if (result.cited_controls) {
    const c = result.cited_controls;
    const totalCited = c.owasp.length + c.nist.length + c.iso42001.length + c.eu_ai_act.length;
    if (totalCited > 0) {
      const allPass = s.auto_pass === s.auto_total && s.auto_fail === 0;
      const header = allPass
        ? 'Controls satisfied (all auto checks PASS):'
        : 'Controls in scope (partial — fix FAIL/OVERCLAIM to fully satisfy):';
      lines.push(`── ${header} ──`);
      if (c.owasp.length > 0) lines.push(`  OWASP LLM Top 10: ${c.owasp.join(', ')}`);
      if (c.nist.length > 0)  lines.push(`  NIST AI RMF:      ${c.nist.join(', ')}`);
      if (c.iso42001.length > 0) lines.push(`  ISO/IEC 42001:    ${c.iso42001.join(', ')}`);
      if (c.eu_ai_act.length > 0) lines.push(`  EU AI Act:        ${c.eu_ai_act.join(', ')}`);
      lines.push('');
    }
  }

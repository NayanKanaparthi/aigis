...
const { getUserJurisdictions, filterAreasByJurisdiction, stripJurisdictionGatedSections } = require('./jurisdiction');
 ...
 */
function buildList({ description, extraJurisdictions = [], tight = false } = {}) {
  ...
  // v2.1: --jurisdiction-derived traits added before classify; jurisdiction filter applied to areas after.
  const extraJurisdictionTraits = (extraJurisdictions || []).filter(Boolean).map((c) => `jurisdiction-${c}`);
  const allTraitsWithJurisdiction = [...new Set([...resolved.all_traits, ...extraJurisdictionTraits])].sort();
  if (allTraitsWithJurisdiction.length === 0) {
  ...
  const cls = classify(allTraitsWithJurisdiction);
  const userJurisdictions = getUserJurisdictions(allTraitsWithJurisdiction);
  let areas = filterAreasByJurisdiction(cls.implement_files, userJurisdictions);
  // v2.1 --tight mode for buildList — same logic as prepareBriefInputs.
  if (tight) {
    const userTraitSet = new Set(allTraitsWithJurisdiction);
    areas = areas.filter((area) => {
      const sysTraits = readAreaSystemTraits(area) || [];
      return sysTraits.filter((t) => userTraitSet.has(t)).length >= 2;
    });
  }
...
function buildCompact({
  ...
  const prep = prepareBriefInputs({ description, decisions, extraJurisdictions, tight });
...
function buildFull({
  ...
  // each universal area's content before inlining. EU users see the section.
  const userJ = new Set(prep.user_jurisdictions);
  sections.push(`## Areas`);
  sections.push('');
  let i = 1;
  for (const area of areas) {
    ...
    let stripped = stripFrontmatterRaw(raw);
    stripped = stripJurisdictionGatedSections(stripped, userJ);
...
function buildBrief(opts = {}) {
  // opts may include: description, decisions, extraJurisdictions, mode, timestamp, versionString
 ...
 *
 * v2.1: applies jurisdiction filter to the area list before infra collection.
 * Universal areas pass through; areas declaring `jurisdiction: [eu]` (etc.) are
 * dropped if the user is not in that jurisdiction. extraJurisdictions augments
 * the resolver-derived jurisdiction set (powers `--jurisdiction <code>` flag).
 */
function prepareBriefInputs({ description, decisions = {}, extraJurisdictions = [], tight = false }) {
  const normalized = normalizeDescription(description);
  if (!normalized) {
    throw new Error('Provide a description in quotes. Example: aigis build "customer chatbot with RAG"');
  }
  ...
  // v2.1: --jurisdiction flag adds jurisdiction-* traits to the set so that
  // resolver-blind users can declare jurisdiction explicitly.
  const extraJurisdictionTraits = (extraJurisdictions || [])
    .filter(Boolean)
    .map((c) => `jurisdiction-${c}`);
  const allTraitsWithJurisdiction = [...new Set([...resolved.all_traits, ...extraJurisdictionTraits])].sort();
  ...
  // v2.1 jurisdiction filter: drop EU-tagged areas if user not in EU, etc.
  const userJurisdictions = getUserJurisdictions(allTraitsWithJurisdiction);
  const areasBeforeFilter = cls.implement_files;
  let areas = filterAreasByJurisdiction(areasBeforeFilter, userJurisdictions);

  // v2.1 --tight mode: require ≥2 of the user's traits to match an area's
  // system_traits before including it. Default mode (≥1) unchanged. Tight
  // mode is opt-in: meant for users who want minimal briefs and accept the
  // recall trade. Benchmarked against default in v2.1-tight harness.
  let areas_before_tight_filter = null;
  let tight_dropped = [];
  if (tight) {
    areas_before_tight_filter = areas.slice();
    const userTraitSet = new Set(allTraitsWithJurisdiction);
    const kept = [];
    for (const area of areas) {
      const sysTraits = readAreaSystemTraits(area) || [];
      const matchCount = sysTraits.filter((t) => userTraitSet.has(t)).length;
      if (matchCount >= 2) kept.push(area);
      else tight_dropped.push({ area, match_count: matchCount });
    }
    areas = kept;
  }
  ...
  return {
    ...
    cls,
    areas,
    areas_before_jurisdiction_filter: areasBeforeFilter,
    areas_before_tight_filter,    // null if tight mode was off
    tight_dropped,                 // [] if tight mode was off
    tight_mode: !!tight,
...
/**
 * v2.1 helper: read an area's `system_traits` frontmatter. Used by tight
 * mode to count user-trait matches per area. Returns null on missing file
 * or parse failure (caller treats as 0 matches → area dropped under tight).
 */
function readAreaSystemTraits(areaId) {
  try {
    const filepath = path.join(CONTENT_DIR, 'skills', 'areas', `${areaId}.md`);
    if (!fs.existsSync(filepath)) return null;
    const fm = matter(fs.readFileSync(filepath, 'utf8')).data;
    return Array.isArray(fm.system_traits) ? fm.system_traits : null;
  } catch (_) {
    return null;
  }
}

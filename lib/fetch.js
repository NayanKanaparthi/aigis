...
function getInfra(area) {
  if (!area) {
    throw new Error('Provide an infrastructure area. Run "aigis infra --list" to see available areas.');
  }
  return readFile('skills/infrastructure', area);
}

function listInfras() {
  // Sort case-insensitively so output is stable regardless of filesystem listing order.
  return listDir('skills/infrastructure').sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase()));
}

module.exports = { get, verify, getTemplate, getAuditScan, getCore, getWorkflow, listWorkflows, getInfra, listInfras, readFile, listDir };

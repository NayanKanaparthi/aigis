...
2. For EACH recommended file in implement_files, work one at a time:
   a) Fetch one file:
      Run: aigis get <file-id>
   b) Implement the patterns in code
   c) Verify that same file:
      Run: aigis verify <file-id>
   d) Fix FAIL/PARTIAL items until all checks pass
   e) Move to the next file

3. After all files pass, generate compliance docs:
...
4. To audit an existing project:
